...
if (orient == 'z')
    ...
    if (ceil(index) == floor(index))
        DOSE2D = reshape(DOSE3D(:,:,index),[length(x),length(y)]);
    else
        Plane1 = reshape(DOSE3D(:,:,floor(index)),[length(x),length(y)]);
        Plane2 = reshape(DOSE3D(:,:,ceil(index)),[length(x),length(y)]);
        DOSE2D = reshape(Plane1 + (Plane2 - Plane1)*(index-floor(index)),[length(x),length(y)]);
    end
end